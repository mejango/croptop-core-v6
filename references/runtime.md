...
- [`src/CTPublisher.sol`](../src/CTPublisher.sol) validates posts, configures or reuses tiers, mints first copies, routes Croptop fees, and records optional CPN referral volume.
...
3. The publisher validates category-level rules, creates or reuses tiers, mints the first copy, routes fees and proceeds, and credits CPN referral volume when a supplied referral key maps to a successfully paid fee.
...
- Fee routing: fee-project assumptions, fee exemptions, and referral-volume credits are operationally important.
