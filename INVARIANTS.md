...
- **A.4.2 Croptop ALWAYS prepends its own canonical `(true, tierIdsToMint)` `pay` entry.** After the existence check, `mintFrom` calls `JBMetadataResolver.addToMetadata` to append its own pay entry derived from `_setupPosts`' output. The first 32 bytes of the assembled metadata are overwritten with `FEE_PROJECT_ID` for fee-project attribution (`src/CTPublisher.sol`). The 721 hook only sees the Croptop-authored tier list.
