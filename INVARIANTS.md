...
- **`CTPublisher`** is the universal entrypoint: any caller (subject to per-category allowlist) can call `mintFrom` to create one or more new 721 tiers on a project and mint the first copy in a single transaction. The caller supplies a terminal `token` and `amount`; native-token payments use `msg.value`, and ERC-20 payments are pulled from `_msgSender()`. Tier creation is gated by per-category `minimumPrice`, `minimumTotalSupply`, `maximumTotalSupply`, `maximumSplitPercent` set by the project owner (or an `ADJUST_721_TIERS` operator). A 5% (`1/FEE_DIVISOR=20`) fee routes to the configured fee project. Duplicate IPFS URIs within a batch revert, the second mint of an already-published URI reuses the existing tier rather than creating a duplicate, and a project payment that does not deliver the requested NFTs reverts.
...
- **A.1.2 Posts must satisfy ALL four per-category bounds, evaluated together.** For each NEW tier (`tierIdsToMint[i] == 0`), `_setupPosts` reads the packed allowance via `allowanceFor` and enforces:
  - `post.price >= minimumPrice` else `CTPublisher_PriceTooSmall` (`src/CTPublisher.sol:730-733`).
  - `post.totalSupply >= minimumTotalSupply` else `CTPublisher_TotalSupplyTooSmall` (`src/CTPublisher.sol:735-740`).
  - `post.totalSupply <= maximumTotalSupply` (when `maximumTotalSupply != 0`; zero means unlimited) else `CTPublisher_TotalSupplyTooBig` (`src/CTPublisher.sol:743-749`).
  - `post.splitPercent <= maximumSplitPercent` else `CTPublisher_SplitPercentExceedsMaximum` (`src/CTPublisher.sol:751-755`).
  All four are enforced independently — none can override another.
- **A.1.3 Posting to an unconfigured / disabled category reverts.** `_setupPosts` checks `minimumTotalSupply == 0` and reverts `CTPublisher_UnauthorizedToPostInCategory` (`src/CTPublisher.sol:725-728`). Since `configurePostingCriteriaFor` itself rejects `minimumTotalSupply == 0` (`src/CTPublisher.sol:159-162`, B.1.3), a category is "active" exactly when its operator has explicitly configured it.
- **A.1.4 Empty `posts` array reverts.** `mintFrom` reverts `CTPublisher_NoPosts` if `posts.length == 0` (`src/CTPublisher.sol:347-348`). A caller cannot use Croptop as a fee-free pay-router to the project terminal.

## A.2 Duplicate-IPFS protection

- **A.2.1 Within-batch duplicate IPFS URIs revert.** `_setupPosts` runs an O(n²) inner loop comparing each post's `encodedIpfsUri` against all earlier indices in the same batch; a match reverts `CTPublisher_DuplicatePost` (`src/CTPublisher.sol:675-679`). Posters cannot batch the same URI twice to pay for one tier-creation but mint two NFTs at the new tier's price (the fee path uses `totalPrice`, not `numberOfNewTiers`).
- **A.2.2 Empty IPFS URI reverts.** Any post with `encodedIpfsUri == bytes32(0)` reverts `CTPublisher_EmptyEncodedIpfsUri` (`src/CTPublisher.sol:669-672`). Prevents the canonical "missing URI" tier from being claimed by anyone.
- **A.2.3 Cross-batch duplicate reuses the existing tier at its existing price.** When a previously-published URI is included again, `_setupPosts` looks up `tierIdForEncodedIpfsUriOf[hook][uri]`. If the cached tier still exists and its URI still matches, the function uses the CACHED tier's stored `price` (NOT the caller-supplied `post.price`) when accumulating `totalPrice` and routes the mint to the existing tier (`src/CTPublisher.sol:687-707`). A caller cannot pass `post.price = 0` for an existing tier to mint at zero cost.
- **A.2.4 Stale cache is auto-invalidated.** If the cached tier was removed via `adjustTiers` (`store.isTierRemoved == true`) OR its URI changed via `setMetadata` (`cachedTier.encodedIpfsUri != post.encodedIpfsUri`), `_setupPosts` deletes the mapping and falls through to create a new tier (`src/CTPublisher.sol:690-701`). Stale mappings cannot strand a URI or charge stale prices.

## A.3 Fee accounting and routing

- **A.3.1 5% fee on `totalPrice`, payable in the selected terminal token.** `mintFrom` takes `token` and `amount`. Native-token payments require ETH/native-token-alias 18-decimal tier pricing; ERC-20 payments require the project terminal's accounting context for `token` to match `hook.pricingContext()` (`src/CTPublisher.sol:209-239, 563-600`). When `projectId != FEE_PROJECT_ID`, it deducts `fee = totalPrice / FEE_DIVISOR` (with `FEE_DIVISOR = 20`, i.e. 5%) from `amount`. The remainder (`payValue`) is paid to the project's primary terminal for `token`; the fee is paid to the fee project's primary terminal for `token` (`src/CTPublisher.sol:381-399, 445-495`).
- **A.3.2 Insufficient `amount` reverts.** Two separate checks: (a) `payValue < fee` (the fee deduction would underflow) reverts `CTPublisher_InsufficientEthSent` for native token or `CTPublisher_InsufficientPayment` for ERC-20 (`src/CTPublisher.sol:386-391, 618-624`); (b) `totalPrice > payValue` (after fee deduction, remaining amount cannot cover the tier prices) reverts the same token-appropriate error (`src/CTPublisher.sol:396-399, 618-624`).
...
- **A.3.4 Fee project posts skip the fee.** When `projectId == FEE_PROJECT_ID`, the fee deduction and fee-terminal call are skipped entirely (`src/CTPublisher.sol:381-394, 474-496`). The fee project does not pay a fee to itself.
- **A.3.5 Failed fee payment refunds the caller.** The fee terminal call is wrapped in try/catch. If the fee terminal reverts (e.g. fee-project terminal misconfiguration), the fee amount is refunded to `_msgSender()` in the selected token. If a native-token refund fails, the whole tx reverts `CTPublisher_FeePaymentFailed` (`src/CTPublisher.sol:481-495, 543-555`). Fees never strand inside `CTPublisher`.
- **A.3.6 `feeBeneficiary == address(0)` rejected upfront.** Reverts `CTPublisher_InvalidFeeBeneficiary` (`src/CTPublisher.sol:350-351`). Prevents callers from burning the fee project's tokens by minting to the zero address.
...
- **B.1.1 `configurePostingCriteriaFor` is gated by the hook owner's permission table.** For each entry, `CTPublisher` calls `_requirePermissionFrom({account: JBOwnable(hook).owner(), projectId: hook.projectId(), permissionId: JBPermissionIds.ADJUST_721_TIERS})` (`src/CTPublisher.sol:152-157`). Until the project owner claims hook ownership (see B.4), `JBOwnable(hook).owner()` resolves to `CTDeployer`; afterwards it resolves dynamically through `PROJECTS.ownerOf(projectId)`. Either way, only the legitimate authority for the hook can edit criteria.
- **B.1.2 Each call REPLACES the criteria for the specified `(hook, category)`.** The packed allowance is overwritten in one `SSTORE`, and the allowlist array is either bulk-assigned (non-empty supplied) or `delete`d (empty supplied) (`src/CTPublisher.sol:172-182, 186-190`). Stale allowlist entries cannot survive across edits.
- **B.1.3 `minimumTotalSupply == 0` rejected.** Reverts `CTPublisher_ZeroTotalSupply` (`src/CTPublisher.sol:159-162`). Operators cannot configure a category that would simultaneously appear "active" (allowing posts) and "inactive" (failing A.1.3). The internal-state invariant `_packedAllowanceFor[hook][category] != 0  iff  category is active` is preserved.
- **B.1.4 `minimumTotalSupply > maximumTotalSupply` rejected (when max != 0).** Reverts `CTPublisher_MaxTotalSupplyLessThanMin` (`src/CTPublisher.sol:164-170`). `maximumTotalSupply == 0` is treated as unlimited and skips the check, matching the runtime semantics in A.1.2.
...
- **B.3.2 No fee redirection.** `FEE_PROJECT_ID` is `immutable` on `CTPublisher` (`src/CTPublisher.sol:82-83, 113-128`). The operator cannot redirect the 5% fee to themselves.
...
- **`constructor(IJBDirectory directory, IJBPermissions permissions, uint256 feeProjectId, address trustedForwarder)`** (`src/CTPublisher.sol:117-128`) — sets `DIRECTORY`, `FEE_PROJECT_ID`, ERC-2771 trusted forwarder, and `JBPermissioned`'s `PERMISSIONS`. All four are immutable.

### Operator-gated configuration

- **`configurePostingCriteriaFor(CTAllowedPost[] memory allowedPosts) external`** (`src/CTPublisher.sol:141-196`) — gated per-entry by `_requirePermissionFrom(JBOwnable(hook).owner(), hook.projectId(), ADJUST_721_TIERS)`. Iterates the array; for each entry validates `minimumTotalSupply != 0` and `minimumTotalSupply <= maximumTotalSupply` (when max != 0), then packs `(minimumPrice | minimumTotalSupply | maximumTotalSupply | maximumSplitPercent)` into one slot and writes the allowlist array. Emits `ConfigurePostingCriteria` per entry. Reverts: `CTPublisher_ZeroTotalSupply`, `CTPublisher_MaxTotalSupplyLessThanMin`, plus `_requirePermissionFrom`'s unauthorized revert.
  - **Invariants:** B.1.1, B.1.2, B.1.3, B.1.4, B.1.5, B.2.1.

### Permissionless mint entrypoint

- **`mintFrom(IJB721TiersHook hook, CTPost[] calldata posts, address token, uint256 amount, address nftBeneficiary, address feeBeneficiary, bytes calldata additionalPayMetadata) external payable`** (`src/CTPublisher.sol:209-239, 336-497`) — anyone. Validates native-token `amount == msg.value` or pulls ERC-20 `amount` from `_msgSender()`; validates `posts.length != 0`, `feeBeneficiary != address(0)`, and payment-token accounting context against hook pricing; calls `_setupPosts` to validate per-category allowances, build sorted tier configs, accumulate `totalPrice`; computes fee (skipping when `projectId == FEE_PROJECT_ID`); validates `amount` covers price+fee; calls `hook.adjustTiers(tiersToAdd, [])` to add new tiers; rejects `additionalPayMetadata` containing a `pay`-purpose ID; composes Croptop's canonical pay metadata with `FEE_PROJECT_ID` referral; emits `Mint`; pays the project's primary terminal for `token`; verifies the beneficiary received the requested NFTs; recomputes the fee from `amount - payValue`; if fee != 0, try/catch pays the fee project's primary terminal for `token`, clearing and refunding on catch.
  - Reverts: `CTPublisher_NoPosts`, `CTPublisher_InvalidFeeBeneficiary`, `CTPublisher_InvalidPaymentTokenContext`, `CTPublisher_NativeTokenAmountMismatch`, `CTPublisher_MsgValueNotAllowed`, `CTPublisher_InsufficientEthSent` / `CTPublisher_InsufficientPayment`, `CTPublisher_TemporaryAllowanceNotConsumed`, `CTPublisher_DuplicatePayMetadata`, `CTPublisher_MintNotDelivered`, `CTPublisher_FeePaymentFailed`, plus all `_setupPosts` reverts below.
  - **Invariants:** A.1.1–A.1.4, A.2.1–A.2.4, A.3.1–A.3.8, A.4.1–A.4.2, A.5.1–A.5.2.

### Views

- **`tiersFor(address hook, bytes32[] encodedIpfsUris) external view → JB721Tier[]`** (`src/CTPublisher.sol:253-280`) — looks up `tierIdForEncodedIpfsUriOf[hook][uri]` per URI and queries the hook's store. Returns the default empty tier when not yet published OR when the cached ID is stale; callers must check `tier.initialSupply` to disambiguate (documented in source).
- **`allowanceFor(address hook, uint256 category) public view → (uint256, uint256, uint256, uint256, address[])`** (`src/CTPublisher.sol:294-322`) — unpacks the four bit-fields and returns the allowlist.
...
- **`_clearAllowanceFor`, `_prepareAllowanceFor`, `_requireTemporaryAllowanceConsumed`** (`src/CTPublisher.sol:506-510, 537-541, 605-611`) — manage exact-use temporary ERC-20 allowances for downstream terminal calls.
- **`_isAllowed(address addr, address[] addresses) internal pure → bool`** (`src/CTPublisher.sol:518-531`) — O(n) linear scan.
- **`_refundFee`** (`src/CTPublisher.sol:546-555`) — refunds a failed fee payment in the selected token; native-token refund failure reverts.
- **`_requirePaymentTokenMatchesPricing`** (`src/CTPublisher.sol:563-600`) — validates native pricing or ERC-20 terminal accounting context against the hook pricing context.
- **`_revertInsufficientPayment`** (`src/CTPublisher.sol:618-624`) — preserves the native-compatible insufficient-ETH error for native payments and uses token-generic insufficient-payment errors otherwise.
...
- **`_contextSuffixLength` / `_msgData` / `_msgSender`** — standard ERC-2771 trampolines (`src/CTPublisher.sol:854-868`).
...
- **E.4 `CTPublisher.FEE_PROJECT_ID` is immutable.** Set at constructor time, never mutable (`src/CTPublisher.sol:82-83, 113-128`). If the configured fee project is misconfigured for the selected payment token at the time of a mint, the try/catch in A.3.5 refunds the caller — the protocol fee for that mint is effectively waived. There is no upgrade or migration path.
...
|---|---|
| A.1.1 (allowlist-empty implies open posting) | `src/CTPublisher.sol:518-531, 758-760` |
| A.1.2 (four per-category bounds) | `src/CTPublisher.sol:730-755` |
| A.1.3 (unconfigured category reverts) | `src/CTPublisher.sol:725-728` |
| A.1.4 (empty posts reverts) | `src/CTPublisher.sol:347-348` |
| A.2.1 (within-batch duplicate URI) | `src/CTPublisher.sol:675-679` |
| A.2.2 (empty URI rejected) | `src/CTPublisher.sol:669-672` |
| A.2.3 (cross-batch reuses cached tier, cached price) | `src/CTPublisher.sol:687-707` |
| A.2.4 (stale cache invalidation) | `src/CTPublisher.sol:690-701` |
| A.3.1 (5% fee on totalPrice) | `src/CTPublisher.sol:71-73, 381-399, 445-495, 563-600` |
| A.3.2 (insufficient amount) | `src/CTPublisher.sol:386-399, 618-624` |
| A.3.4 (fee-project self-skip) | `src/CTPublisher.sol:381-394, 474-496` |
| A.3.5 (failed-fee refund) | `src/CTPublisher.sol:481-495, 546-555` |
| A.3.6 (feeBeneficiary != 0) | `src/CTPublisher.sol:350-351` |
| A.3.7 (post-call fee math via amount - payValue) | `src/CTPublisher.sol:470-472` |
| A.4.1 (caller can't include pay-purpose metadata) | `src/CTPublisher.sol:407-412` |
| A.4.2 (Croptop's canonical pay metadata) | `src/CTPublisher.sol:415-427` |
| A.5.1 (category sort + post-index lockstep) | `src/CTPublisher.sol:649-658, 799-835` |
| B.1.1 (configurePostingCriteriaFor permission gate) | `src/CTPublisher.sol:152-157` |
| B.1.2 (per-call replacement) | `src/CTPublisher.sol:172-182, 186-190` |
| B.1.3 (minimumTotalSupply != 0) | `src/CTPublisher.sol:159-162` |
| B.1.4 (min ≤ max when max != 0) | `src/CTPublisher.sol:164-170` |
| B.1.5 (no full-disable, only restriction) | `src/CTPublisher.sol:138-140` |
...
| D.1 (tier-per-post fixed flags) | `src/CTPublisher.sol:764-784` |
...
| D.7 (fee project credit) | `src/CTPublisher.sol:474-495` |
| D.9 (ERC-2771 sender resolution) | `src/CTPublisher.sol:854-868`, `src/CTDeployer.sol:498-512` |
...
| E.4 (FEE_PROJECT_ID immutable) | `src/CTPublisher.sol:82-83, 113-128` |
