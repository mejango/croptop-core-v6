...
- **`CTPublisher`** is the universal entrypoint: any caller (subject to per-category allowlist) can call `mintFrom` to create one or more new 721 tiers on a project and mint the first copy in a single transaction. The caller supplies a terminal `token`, `amount`, and `referralProjectId`; native-token payments use `msg.value`, and ERC-20 payments are pulled from `_msgSender()` by direct approval or Permit2. Tier creation is gated by per-category `minimumPrice`, `minimumTotalSupply`, `maximumTotalSupply`, `maximumSplitPercent` set by the project owner (or an `ADJUST_721_TIERS` operator). A 5% (`1/FEE_DIVISOR=20`) fee routes to the configured fee project or refunds if that fee path is unavailable. Successful CPN fee payments can credit normalized referral volume by `referralChainId:referralProjectId`. Duplicate IPFS URIs within a batch revert, the second mint of an already-published URI reuses the existing tier rather than creating a duplicate, and a project payment that does not deliver the requested NFTs reverts.
...
- **A.3.10 CPN referral credit only tracks successfully paid, normalized Croptop fees.** `referralProjectId == 0` records no referral credit. A bare nonzero project ID is packed with `block.chainid`; `(referralChainId << 48) | referralProjectId` credits a project on another chain. After the CPN fee terminal accepts the fee, `_creditFeeReferralFor` unpacks the key, normalizes the fee to native-token units with 18 decimals, increments `feeVolumeByReferralOf[referralChainId][bareProjectId]`, increments `totalFeeVolume`, and emits `ReferralCredit`. Missing fee terminals, fee-terminal reverts, missing accounting context, missing price feeds, zero prices, and zero normalized amounts do not credit referral volume (`src/CTPublisher.sol`).
...
- **A.4.2 Croptop ALWAYS prepends its own canonical `(true, tierIdsToMint)` `pay` entry.** After the existence check, `mintFrom` calls `JBMetadataResolver.addToMetadata` to append its own pay entry derived from `_setupPosts`' output. The first 32 bytes of the assembled metadata are overwritten with `FEE_PROJECT_ID` in the metadata project-id slot (`src/CTPublisher.sol`). The 721 hook only sees the Croptop-authored tier list.
...
- **`mintFrom(IJB721TiersHook hook, CTPost[] calldata posts, address token, uint256 amount, address nftBeneficiary, address feeBeneficiary, bytes calldata additionalPayMetadata, uint256 referralProjectId) external payable`** (`src/CTPublisher.sol`) — anyone. Validates native-token `amount == msg.value` or pulls ERC-20 `amount` from `_msgSender()` by direct allowance or publisher-targeted Permit2 metadata; validates `posts.length != 0`, `feeBeneficiary != address(0)`, and the selected payment token's terminal accounting context; converts hook tier pricing into payment-token units; calls `_setupPosts` to validate per-category allowances, build sorted tier configs, and accumulate `totalPrice`; computes fee (skipping when `projectId == FEE_PROJECT_ID`); validates `amount` covers converted price+fee; calls `hook.adjustTiers(tiersToAdd, [])` to add new tiers; rejects `additionalPayMetadata` containing a `pay`-purpose ID; composes Croptop's canonical pay metadata with `FEE_PROJECT_ID` in the metadata project-id slot; emits `Mint`; pays the project's primary terminal for `token`; verifies the beneficiary received the requested NFTs; recomputes the fee from `amount - payValue`; if fee != 0, pays the fee project's primary terminal for `token` when available and credits `referralProjectId` for normalized CPN fee volume when applicable, otherwise clearing and refunding on failure or missing terminal.
  ...
  - **Invariants:** A.1.1–A.1.4, A.2.1–A.2.4, A.3.1–A.3.10, A.4.1–A.4.2, A.5.1–A.5.2.
...
- **`feeVolumeByReferralOf(uint256 referralChainId, uint256 referralProjectId) public view → uint256`** — auto-getter; normalized CPN fee volume credited to the referral key.
- **`tierIdForEncodedIpfsUriOf(address, bytes32) public view → uint256`** — auto-getter; URI→tierId map.
- **`totalFeeVolume() public view → uint256`** — auto-getter; total normalized CPN fee volume credited across all referral keys.
...
- **`_creditFeeReferralFor`, `_normalizedFeeAmountFrom`** (`src/CTPublisher.sol`) — unpack CPN referral keys, normalize successfully paid fee amounts into native-token units with 18 decimals, update referral-volume totals, and skip credits that cannot be priced reliably.
...
- **D.7 Fee project credit and referral accounting on every fee-paying mint when the fee terminal is available.** `CTPublisher.mintFrom` calls the fee project's primary terminal for the selected `token` with `beneficiary: feeBeneficiary` when that terminal exists and accepts the payment — the fee beneficiary receives the fee project's tokens at the fee project's current weight, NOT the originating revnet's tokens (`src/CTPublisher.sol`). If `referralProjectId` is nonzero, the same successful fee payment can credit normalized CPN fee volume to `feeVolumeByReferralOf[referralChainId][referralProjectId]`. If the fee path is unavailable, the fee is refunded to the caller and no referral volume is credited.
...
| A.3.10 (CPN referral credit tracks paid normalized fees) | `src/CTPublisher.sol` |
...
| D.7 (fee project credit and referral accounting) | `src/CTPublisher.sol` |
